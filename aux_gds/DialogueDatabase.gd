...
func _ready() -> void:
	...
	if not dir.open(SOURCE_DIRECTORY) == OK:
		print("Couldn't read directory %s" % SOURCE_DIRECTORY)
	dir.list_dir_begin()
...
func get_texture(character_name : String, expression = "neutral") -> Texture:
	"""
	Returns the Texture corresponding to a character's name and expression name
	"""
	#assert
	character_name in characters
	#assert
	expression in characters[character_name].expressions
